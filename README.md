...
The complete English LaTeX manuscript is `final.tex`. A School-template-aligned copy of the user-supplied complete manuscript is maintained as `skeleton_repaired.tex`, which is the authoritative file for the current submission draft: the 2026-08-12 `dissertation.pdf` was compiled from it. On 2026-08-16 the manuscript was re-synced from the user's updated export (`E:\download\skeleton (2).tex`); the copy-edit pass (precision fixes for the Horizon structure, metric scopes, human-reference sample wording, equation notation, and bibliography publisher fields) was re-applied, and a content pass addressed the reviewer feedback on Results interpretation (per-figure key-pattern prose and imprecision/interval-straddle counts), on the 20-runs-per-cell rationale and an intuitive account of the random-exploration model, and on Discussion (framework transferability to multimodal/embodied systems and the explicitly non-normative role of the human reference). All newly added summary counts were verified against `outputs/processed/final_analysis_v03`. A line-break/overflow pass then fixed the horizontal overfull lines flagged in the rendered PDF: long `\texttt` settings were split (e.g., `\texttt{reasoning\_effort} = none`), the BART explosion equation was shortened with the `k=1,...,32` range moved to prose, the repository URL and appendix paths now use `\sloppy` wrapping, `\emergencystretch=2em` was added to reduce residual overfull body lines, and three over-long section headings (2.1, 2.4, 4.5) were shortened so they no longer exceed the text width. A final supervisor-response pass added participant-level human-reference bootstrap uncertainty (`outputs/processed/final_analysis_v03/human_reference_results/human_reference_bootstrap.csv`, produced by `src/build_human_reference_uncertainty.py`), replaced the RQ summary table with a slimmer scan-friendly version, and turned the Discussion transfer paragraph into an explicit minimal transfer checklist. Both files use the confirmed title *How Reliable Are LLMs as Cognitive Models? A Systematic Evaluation of Prompt Sensitivity Across Decision-Making Tasks*. Their preambles retain `infthesis` with `msccheck`, use only functional formula, table, figure, rotation, and URL packages, and omit `microtype` because it triggers the School checker's `\showhyphens` guard. The long repository URL is set in its own locally ragged-right block so that it can wrap without changing the dissertation's global layout. The manuscript is a polished British-academic-English translation of the current authoritative Chinese manuscript,
...
- To prevent horizontal `Overfull \hbox` errors in the School template, the appendix no longer repeats deep GitHub URLs containing the long repository slug. It refers to the single repository-root link in Reproducibility and displays only slash-delimited repository-relative paths via `\url{...}`, allowing LaTeX to wrap them safely at directory separators.
- Statistical Analysis now explicitly distinguishes the eight primary metrics: seven are calculated directly from complete runs, while Random exploration is the eighth and is estimated using the hierarchical logistic choice model. The obsolete reference to a separate technical appendix has been replaced with an accurate pointer to the implementation-level fit and optimiser diagnostics in the repository package indexed by Appendix A.2.
- The manuscript citation pass now separates compound claims from citation clusters. Introduction and Background citations are attached to the specific empirical, methodological, or conceptual claim they support; the three-task and three-dataset source groups remain because they are explicit one-to-one mappings. Discussion now uses a small set of targeted citations to state how the findings extend static multi-template robustness work, model/snapshot dependence, multilingual inconsistency evidence, and synthetic-participant critiques, without repeating the literature review.
