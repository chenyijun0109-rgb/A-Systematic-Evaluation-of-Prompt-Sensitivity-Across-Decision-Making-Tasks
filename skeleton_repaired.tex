...
I would like to thank my supervisor, [SUPERVISOR NAME], for their guidance,
feedback, and support throughout this project. I am also grateful to
[OPTIONAL: ANYONE ELSE WHO PROVIDED SUBSTANTIVE ACADEMIC OR PERSONAL SUPPORT].
...
LLMs are increasingly studied as behavioural systems: they have been used to simulate respondents, reproduce experimental effects, act as economic agents, and perform cognitive tasks \cite{Argyle2023OutOfOneMany,Aher2023SimulateHumans,Horton2023HomoSilicus,Binz2023CognitivePsychologyGPT3,Hagendorff2023HumanLikeBiases}. Their responses can be generated repeatedly under controlled task manipulations, but one run is not a human participant and behavioural similarity does not establish a shared mechanism \cite{Shanahan2023RolePlay}. Here, \emph{cognitive model} means a computational model that produces measurable behaviour in cognitive tasks, not a system assumed to share human cognition.
...
Behavioural research uses LLMs either to simulate human participants or as experimental subjects in their own right \cite{Dillion2023ReplaceParticipants,Hagendorff2023MachinePsychology,Rahwan2019MachineBehaviour}. The first requires valid population inference; the second characterises a specified computational system. This study adopts the latter and defines an LLM operationally as a model producing measurable cognitive-task behaviour, without assuming human perceptual, learning, or decision mechanisms \cite{Lampinen2024ContentEffects,Lin2025SixFallacies}.
...
Behavioural evaluation concerns distributions and trajectories as well as accuracy. Multi-template studies show that one prompt can overstate certainty and that scoring may create apparent variation \cite{Sclar2024PromptFormatting,Mizrahi2024MultiPrompt,Hua2025FlawArtifact}. Here, \emph{behavioural-measurement reliability} means stability in a conclusion's direction and magnitude across prespecified formulations with model snapshot, task, temperature, response format, and parser fixed. It excludes long-term test--retest reliability, external validity, and claims about mechanisms.

The measurement procedure includes the prompt, interface, visible history, output constraint, parser, temperature, and deployment state \cite{Webson2022PromptMeaning,Sclar2024PromptFormatting,Loya2023PromptSensitivityDecision,Chen2024ChatGPTBehavior}. Fixing the parser reduces ambiguity but not effects of length, salience, or position. Complete runs are analytical replicates, not independent synthetic persons, because they share parameters and deployment conditions \cite{Dillion2023ReplaceParticipants,Bisbee2024SyntheticSurveyData}. Prompt stability, task-specific outcomes, and human-reference proximity are therefore reported separately: stability is not quality, and mean proximity is not distributional or mechanistic equivalence \cite{Lampinen2024ContentEffects,Lin2025SixFallacies}.

\section{Prompt Sensitivity and Behavioural Variation}

Labels, demonstration order, formatting, and option order can alter outputs, while misleading instructions may still yield high scores \cite{Zhao2021Calibrate,Lu2022PromptOrder,Sclar2024PromptFormatting,Pezeshkpour2024OptionOrder,Webson2022PromptMeaning}. Multi-prompt evaluation therefore replaces the search for one best prompt \cite{Mizrahi2024MultiPrompt,Zhuo2024ProSA}, but most evidence is static; Loya et al.~\cite{Loya2023PromptSensitivityDecision} examined one sequential task without multilingual comparison.
...
Baselines and prompt sensitivity can differ across models \cite{Sclar2024PromptFormatting,Mizrahi2024MultiPrompt,Zhuo2024ProSA}; comparing one-condition means cannot separate them. The appropriate contrast first estimates each model's condition-minus-Neutral effect and then compares effects directly.
...
In all three tasks, choices and feedback form a trajectory, making the complete run the analysis unit rather than treating trials as independent participants \cite{Wilson2014,Bechara1994,Lejuez2002,Loya2023PromptSensitivityDecision}.
...
In IGT, the same deck preference may arise from different valuation, learning, and consistency processes \cite{Bechara1994,Busemeyer2002PVL,Steingroever2013PVLDelta}; construct-validity reviews caution against a unitary ability score \cite{Buelow2009IGTValidity}. Explicit cumulative information may also reduce LLM memory demands, so interpretation is limited to aligned metrics.

BART pumps may reflect both risk-taking and learning or response consistency \cite{Lejuez2002,Wallsten2005BARTModel,VanRavenzwaaij2011BART}; adjusted pumps, explosions, and post-explosion adjustment are therefore complementary. Because task constructs, scales, and feedback differ, no cross-task ``decision-making ability'' score is constructed.

\section{Human-Reference Comparisons: Purpose and Limits}

Human-reference comparisons distinguish mean position, empirical-interval coverage, and response to manipulation. These answer different questions: similar aggregate output does not imply identical task behaviour or mechanism \cite{Lampinen2024ContentEffects,Lin2025SixFallacies}. An LLM run can align operationally with a participant summary but is not an independent human; task length, presentation, and familiarisation further constrain comparison \cite{Dillion2023ReplaceParticipants,Bisbee2024SyntheticSurveyData,Wilson2014,Buelow2009IGTValidity,Wallsten2005BARTModel}.
...
Synthetic-participant, machine-psychology, prompt-robustness, and multilingual studies pursue different objectives and do not jointly evaluate measurement stability, task-specific behaviour, and human-reference proximity \cite{Dillion2023ReplaceParticipants,Hagendorff2023MachinePsychology,Mizrahi2024MultiPrompt,Zhang2023M3Exam}. Multi-model or multi-prompt evidence remains concentrated in static benchmarks or one sequential task, while multilingual consistency studies use other benchmark settings \cite{Loya2023PromptSensitivityDecision,Sclar2024PromptFormatting,Qi2023CrossLingualConsistency}.

Within the reviewed literature, no identified design combines full sequential runs, controlled prompt variants, multiple models and decision constructs, and a fixed-model multilingual comparison. This study integrates those elements \cite{Loya2023PromptSensitivityDecision,Sclar2024PromptFormatting,Mizrahi2024MultiPrompt} to test whether cognitive-task conclusions remain robust, not whether LLMs are globally human-like.
...
Each Horizon run contained 40 two-option games \cite{Wilson2014}. Four forced choices preceded one free choice in H1 or six in H6. The model saw game and choice numbers, choices remaining, reward history, and any forced option, but not internal condition labels.
...
\]
Explosion was certain at pump 32. The model knew only that risk increased and balloons could differ; it saw the current count and value, total, previous and five recent outcomes, cumulative explosions, and mean pumps. Seeds fixed 40 latent explosion points.
...
\midrule
Horizon & Information-seeking choice rate & Less-observed option chosen on the first free unequal-information choice. \\
Horizon & Horizon-related exploration change & H6 minus H1 exploratory-choice rate; pre-choice mean ties excluded. \\
...
BART & Post-explosion adjustment & Pumps on the next balloon minus pumps on the exploded balloon. \\
\bottomrule
\end{tabularx}
\end{table}

Adjusted pumps conditions on balloon survival and was therefore interpreted with explosion rate. Information-seeking rate is a level measure, not the classic horizon-dependent directed-exploration effect. For run $i$, let $E_{i,h}$ denote the proportion of eligible first free choices selecting the option with the lower pre-choice observed mean at horizon $h$. Equal- and unequal-information games were included and mean ties excluded; the horizon-related change was
\[
...
Random exploration used only the first free choice. For the probability that run $i$ selected A in game $g$ at horizon $h$,
\[
\begin{aligned}
\operatorname{logit}(p_{ig})&=\beta_{i,h}(\Delta R_{ig}+\gamma_h\Delta I_{ig}+\delta_h),\\
\beta_{i,h}&=\exp(\alpha_h+u_{i,h}),\qquad \sigma_{i,h}=1/\beta_{i,h},\\
\mathrm{RE}_{i}&=\sigma_{i,\mathrm{H6}}-\sigma_{i,\mathrm{H1}}.
\end{aligned}
\]
$\Delta R$ is A minus B observed mean reward; $\Delta I$ is 1 when A is less observed, $-1$ when B is less observed, and 0 otherwise. $\gamma_h$ is the information bonus and $\delta_h$ the label bias. Although the code names $\beta$ reward sensitivity, it scales the complete evidence expression and acts as inverse temperature. The cell estimate used $\beta_h^{*}=\exp(\alpha_h)$; run summaries used the jointly estimated, partially pooled $\mathrm{RE}_i$. Positive RE denotes greater estimated choice variability at H6, not proof of psychological randomness. MAP estimation used Gaussian run-effect shrinkage (primary SD 0.5; sensitivity 0.25 and 1.0), and relevant bootstrap replicates refitted the model. Full parameterisation and optimisation details are reported in the technical appendix.
...
All human-reference quantities were point estimates against fixed datasets; their reference uncertainty was not propagated, and they do not establish population equivalence or shared mechanisms.
...
\begin{tabularx}{\textwidth}{@{}p{1.0cm}Xp{4.2cm}@{}}
\toprule
RQ & Main empirical pattern & Boundary \\
\midrule
RQ1 & Prompt effects varied across models, tasks, metrics, and formulations. & No universal formulation ranking. \\
RQ2 & Model-by-prompt differences were concentrated in particular outcomes. & Not a capability comparison. \\
RQ3 & Baselines and prompt responses varied jointly across three languages. & Language-associated, not purely causal. \\
RQ4 & Mean deviation and empirical coverage did not always move together. & Descriptive human reference only. \\
\bottomrule
\end{tabularx}
\end{table}
...
The analysis included 1,200 unique complete runs, 20 per cell (Table~\ref{tab:results_quality}); shared English GPT-4.1 runs were counted once. All multilingual runs parsed successfully, and one Mini run succeeded after the prespecified retry. Three other failed attempts retained identifiers, status, time, and replacement mapping but not raw payloads.

All cells passed completeness checks and all formal contrasts obtained 2,000/2,000 valid bootstrap estimates; no pooled cell SD was zero. Intervals describe uncertainty rather than multiplicity-adjusted confirmatory tests. Random-exploration and sensitivity fits converged under aligned human/LLM specifications, but shrinkage changed magnitude, so their standardised and human-reference results remain model-dependent. Table~\ref{tab:rq_summary} summarises the RQ-level conclusions.

\section{RQ1: Within-Model Prompt Sensitivity}
Figure~\ref{fig:rq1} plots signed Hedges' $g$ and its bootstrap interval, whereas the estimates reported here are raw condition-minus-Neutral effects in the original metric units. The distinction matters because a large standardised value can reflect a small within-cell SD rather than a large behavioural change. No formulation produced a common direction or magnitude across all models and outcomes. For GPT-4.1, Role framing reduced Horizon information seeking by 0.0575 (95\% CI [-0.0876,-0.0275]), whereas uncertainty emphasis increased it by 0.1450 ([0.0975,0.1950]). GPT-5.4 Horizon effects were generally nearer zero, while its IGT advantageous-choice rate shifted positively under all three manipulated conditions.
...
\begin{figure}[!tbp]
\centering
\includegraphics[width=0.96\textwidth,height=0.72\textheight,keepaspectratio]{figure_rq1_prompt_effects_overview.png}
...
\begin{figure}[!tbp]
\centering
\includegraphics[width=0.96\textwidth,height=0.72\textheight,keepaspectratio]{figure_rq2_model_interactions_overview.png}
...
\caption{Joint centred three-language comparison. Left: standardised Neutral baselines; right: centred prompt effects. No language is the reference.}
\label{fig:rq3}
\end{figure}

\section{RQ4: Prompt-Associated Changes in Human-Reference Proximity}
Figure~\ref{fig:rq4} contains descriptive point estimates; inferential confidence intervals were not constructed for RQ4. Negative absolute-deviation change denotes movement of the cell mean towards the human mean, whereas positive coverage change denotes greater overlap between run-level values and the empirical human range. Coverage uses the non-missing runs in each cell, giving a resolution of 0.05 when all 20 runs are available. Neither direction is a test of distributional equivalence or shared cognitive mechanism.

No formulation improved both proximity measures uniformly. English GPT-4.1 Horizon uncertainty emphasis increased absolute information-seeking deviation by 1.711 human-SD units and reduced coverage by 0.40. Chinese GPT-4.1 IGT instruction specificity reduced the post-loss-switching deviation by 1.428 while leaving coverage unchanged. Mini BART instruction specificity reduced adjusted-pump deviation by 0.602 and increased coverage by 0.85. The mixture of positive, negative, and near-zero cells confirms that mean location and run-level overlap can change separately; classifications such as ``closer'' refer only to the direction of the absolute-deviation point estimate.

\begin{figure}[!tbp]
\centering
\includegraphics[width=0.96\textwidth,height=0.72\textheight,keepaspectratio]{figure_rq4_human_reference_overview.png}
...
than to the local or Overleaf project filesystem. They are indexed here rather than reproduced verbatim
because the 36 language--task--condition prompt files and complete cell-level
outputs would add substantial length without adding evidence required to follow
the main argument.

\section{Frozen Prompt Materials}

For each task, the four formal conditions were Neutral, Instruction
specificity, Role framing, and Task-specific construct emphasis. Table~\ref{tab:prompt-inventory}
maps the dissertation terminology to the frozen repository files. In the file
paths, \texttt{TASK} is \texttt{bandit}, \texttt{igt}, or \texttt{bart}.

\begin{table}[htbp]
\centering
\caption{Inventory of frozen prompt files.}
\label{tab:prompt-inventory}
\begin{tabularx}{\textwidth}{@{}p{3.2cm}p{3.2cm}X@{}}
\toprule
Language & Condition & Repository path pattern \\
\midrule
English & Neutral & Repository \texttt{prompts/TASK/baseline.md} \\
English & Instruction specificity & Repository \texttt{prompts/TASK/detailed.md} \\
English & Role framing & Repository \texttt{prompts/TASK/role\_human.md} \\
English & Construct emphasis & Repository files named \texttt{uncertainty\_emphasis.md}, \texttt{reward\_loss\_emphasis.md}, and \texttt{risk\_emphasis.md} in the respective task folders \\
Simplified Chinese & All four conditions & Repository \texttt{prompts/multilingual/zh-CN/TASK/} \\
Spanish & All four conditions & Repository \texttt{prompts/multilingual/es/TASK/} \\
\bottomrule
\end{tabularx}
\end{table}

The complete prompt collection is available at:
\begin{center}
\url{https://github.com/chenyijun0109-rgb/A-Systematic-Evaluation-of-Prompt-Sensitivity-Across-Decision-Making-Tasks/tree/main/prompts}
\end{center}
The prompt-development protocol and provenance are in the repository's
\texttt{prompts/generation/} folder, including the dated generation records.
Multilingual constraints and the completed audit trail are in
\texttt{prompts/multilingual/}. These records document informational checks and
...
The authoritative machine-readable analysis package is available online at:
\begin{center}
\url{https://github.com/chenyijun0109-rgb/A-Systematic-Evaluation-of-Prompt-Sensitivity-Across-Decision-Making-Tasks/tree/main/outputs/processed/final_analysis_v03}
\end{center}
Its
\texttt{analysis\_manifest.json} records file hashes, expected row counts,
...
Publication displays and additional detailed figures are available at:
\begin{center}
\url{https://github.com/chenyijun0109-rgb/A-Systematic-Evaluation-of-Prompt-Sensitivity-Across-Decision-Making-Tasks/tree/main/outputs/figures/final_results_v01}
\end{center}
Detailed or appendix-oriented versions are in the
\texttt{detail\_or\_appendix/} subdirectory. These files
support inspection of cells omitted from the necessarily selective Results
prose. The principal findings, methodological contribution, and evidence needed
to assess the dissertation's conclusions remain in the main body.
