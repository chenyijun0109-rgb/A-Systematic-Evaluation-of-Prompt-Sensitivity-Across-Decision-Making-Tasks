...
$resultRoot = Join-Path $projectRoot "outputs\formal_multilingual_v01\gpt-4.1-zh-CN-20run-v01"
        ...
        ForEach-Object {
            try {
                ...
                if (
                    ...
                    $payload.prompt_language -eq "zh-CN" -and
                    $payload.requested_model -eq "gpt-4.1-2025-04-14"
                ) { $count++ }
            } catch {
                # Ignore incomplete JSON; the next repair pass will retry it.
            }
        }
    return $count
}
...
while ($true) {
    ...
    for ($index = 0; $index -lt $incomplete.Count; $index += 2) {
        $pair = @($incomplete[$index..([Math]::Min($index + 1, $incomplete.Count - 1))])
        $workers = foreach ($shard in $pair) {
            Start-Process `
                -FilePath powershell.exe `
                -ArgumentList @(
                    "-NoProfile", "-ExecutionPolicy", "Bypass",
                    "-File", (Join-Path $PSScriptRoot "run_gpt41_chinese_shard.ps1"),
